...
import { getAllVendorsIncludingSeeded, CATEGORIES, SEARCH_PLACEHOLDERS } from '../data.js';
import { icons, openVendorProfileById, refreshIcons } from '../app.js';
import { renderMapDiscoveryCard } from '../components.js';
...
export async function renderMapScreen(container) {
  ...
  initMap(allVendors);
  bindMapEvents(container, allVendors);
...
function renderDiscoveryCards(vendors) {
  ...
  container.innerHTML = vendors.map(v => renderMapDiscoveryCard(v)).join('');
...
function bindMapEvents(container, allVendors) {
  // Category filter
  container.querySelectorAll('.filter-chip[data-cat]').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      const filtered = activeCategory === 'all' ? allVendors : allVendors.filter(v => v.category === activeCategory);
  ...
  document.getElementById('map-near-btn')?.addEventListener('click', () => {
    ...
    const nearby = allVendors.filter(v => (v.distance || 0) <= 5).sort((a,b) => (a.distance || 0) - (b.distance || 0));
    plotVendors(nearby);
    if(nearby.length > 0) {
  ...
  document.getElementById('map-area-btn')?.addEventListener('click', () => {
    ...
    const visible = allVendors.filter(v => bounds.contains([v.lat, v.lng]));
  ...
  document.getElementById('map-search-input')?.addEventListener('input', (e) => {
    ...
    if (q.length < 2) {
      plotVendors(activeCategory === 'all' ? allVendors : allVendors.filter(v => v.category === activeCategory));
      return;
    }
